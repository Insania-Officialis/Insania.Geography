...
using Insania.Geography.Contracts.Services;
using Insania.Geography.Database.Contexts;
using Insania.Geography.Entities;

namespace Insania.Geography.Services;
...
public class LoggingSL: ILoggingSL, IHostedService, IDisposable
{
    ...
    public LoggingSL(IServiceScopeFactory scopeFactory, ILogger<LoggingSL> logger)
    {
        ...
        //Создание канала
        _channel = Channel.CreateBounded<LogApiGeography>(options);
    }
    #endregion
    ...
    /// </summary>
    private readonly Channel<LogApiGeography> _channel;
    ...
    /// Пачка записей
    /// </summary>
    private readonly List<LogApiGeography> _batch = [];
    ...
    public ValueTask QueueLogAsync(LogApiGeography log) => _channel.Writer.WriteAsync(log, _cancellationTokenSource.Token);
    ...
    private async Task SaveBatchAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            LogsApiGeographyContext context = scope.ServiceProvider.GetRequiredService<LogsApiGeographyContext>();
